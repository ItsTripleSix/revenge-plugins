...
replace_once(
'''        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.5.6"),\n        React.createElement(RN.Text, { style: text }, activeThemeText),\n        React.createElement(RN.Text, { style: text }, "Mentioned messages can now use automatic black/white body-text contrast against the chosen highlight background."),''',
'''        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.6.0 TEST"),\n        React.createElement(RN.Text, { style: text }, activeThemeText),\n        React.createElement(RN.Text, { style: text }, "Visual batch test: intentionally loud colors are enabled for new icon and UI-accent targets so they can be validated quickly from screenshots."),''',
'header')

# Header wording in v0.5.6 has a longer sentence; handle exact current text if prior replacement did not hit.
if 'Theme Toolkit v0.5.6' in text:
    text = text.replace('Theme Toolkit v0.5.6', 'Theme Toolkit v0.6.0 TEST', 1)
if 'Mentioned messages can now use automatic black/white body-text contrast against the chosen highlight background. Inline @mention tag styling remains independent.' in text:
    text = text.replace(
