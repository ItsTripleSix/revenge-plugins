(() => {
  ...
  function rawName(value) {
    ...
    return usableName(mutable) ?? usableName(direct) ?? usableName(canonical) ?? null;
  }

  function findChannelDeep(value, depth = 0, seen = new Set()) {
    if (value == null || depth > 4) return null;

    const direct = unwrapChannel(value);
    if (isGuildChannel(direct) && direct?.id) return direct;
    if (typeof direct === "string") {
      const canonical = getCanonicalChannel(direct);
    ...
    for (const key of ["channel", "record", "item", "data", "row", "props"]) {
  ...
  function namedChannelView(channel, name) {
    if (!channel || typeof channel !== "object" || !name) return channel;
    try {
      const view = Object.create(channel);
      Object.defineProperty(view, "name", { value: name, enumerable: true, configurable: true });
      if (typeof channel.isObfuscated === "function") {
        Object.defineProperty(view, "isObfuscated", {
          value: () => false,
          enumerable: false,
          configurable: true,
        });
  ...
  function rewriteRowTree(node, sourceChannel, name, depth = 0) {
    if (!name || depth > 12) return node;
    if (isPlaceholder(node)) return name;

    if (Array.isArray(node)) {
      let changed = false;
      const next = node.map(child => {
        const value = rewriteRowTree(child, sourceChannel, name, depth + 1);
        changed ||= value !== child;
        return value;
      });
    ...
    for (const key of ["channel", "record"]) {
      const candidate = unwrapChannel(props[key]);
      if (!candidate || candidate?.id !== sourceChannel?.id) continue;
      const replacement = namedChannelView(candidate, name);
      if (props[key]?.channel === candidate) {
        overrides[key] = { ...props[key], channel: replacement };
      } else if (props[key]?.record === candidate) {
        overrides[key] = { ...props[key], record: replacement };
      } else {
    ...
    const nextChildren = children === undefined
      ? children
      : rewriteRowTree(children, sourceChannel, name, depth + 1);
  ...
  function patchChannelMode() {
    ...
    return safePatch("channelMode", () => after("getChannelMode", module, (args, result) => {
      ...
      try {
        ...
        if (peerMode !== undefined && peerMode !== null) {
          rt.modeHits += 1;
          return peerMode;
        }
      } catch {}
  ...
  function patchChannelRows() {
    ...
    return safePatch("channelRows", () => after(method, target, (args, result) => {
      ...
      const passedChannel = findChannelDeep(args?.[0]) ?? findChannelDeep(args);
      if (!passedChannel || !isHidden(passedChannel)) return result;

      const name = rawName(passedChannel);
      if (!name) return result;
      rt.rowNames += 1;
      return rewriteRowTree(result, passedChannel, name);
    }));
  }
  ...
  function patchNameHelpers() {
    ...
    if (typeof module.computeChannelName === "function") {
      changed = safePatch("computeChannelName", () => after("computeChannelName", module, (args, result) => {
        if (!rt.active || !isPlaceholder(result)) return result;
        const channel = findChannelDeep(args);
        if (!channel || !isHidden(channel)) return result;
        const name = rawName(channel);
        if (name) rt.helperHits += 1;
        return name ?? result;
      })) || changed;
    }

    if (typeof module.default === "function") {
      changed = safePatch("useChannelName", () => after("default", module, (args, result) => {
        if (!rt.active || !isPlaceholder(result)) return result;
        const channel = findChannelDeep(args);
        if (!channel || !isHidden(channel)) return result;
        const name = rawName(channel);
  ...
  function patchPrivateChannelHiding() {
    ...
    if (typeof module.isChannelMetadataObfuscationEnabled === "function") {
      changed = safePatch("metadataObfuscationEnabled", () => after(
        "isChannelMetadataObfuscationEnabled", module, () => false,
      )) || changed;
    }
    if (typeof module.useIsChannelMetadataObfuscationEnabled === "function") {
      changed = safePatch("metadataObfuscationHook", () => after(
        "useIsChannelMetadataObfuscationEnabled", module, () => false,
      )) || changed;
    }
    if (typeof module.isChannelMetadataIntegrityCheckEnabled === "function") {
      changed = safePatch("metadataIntegrityCheck", () => after(
        "isChannelMetadataIntegrityCheckEnabled", module, () => false,
      )) || changed;
    }
    if (typeof module.getCachedPrivateChannelObfuscation === "function") {
      changed = safePatch("cachedMetadataObfuscation", () => after(
        "getCachedPrivateChannelObfuscation", module, () => false,
      )) || changed;
    }
  ...
  function inspectRawNames() {
    ...
    try {
      ...
      for (const id of Object.keys(guilds)) {
        let channels;
        try { channels = channelStore()?.getMutableGuildChannelsForGuild?.(id); } catch {}
    ...
    const message = [
      ...
      `Channel-row patch active: ${rt.patched.channelRows ? "yes" : "no"}`,
      `Row renders seen / named: ${rt.rowHits} / ${rt.rowNames}`,
      `Mode substitutions: ${rt.modeHits}`,
      `Name-helper substitutions: ${rt.helperHits}`,
      examples.length ? `\nExamples:\n${examples.join("\n")}` : "",
      "",
  ...
  function Settings() {
    return React.createElement(
      ...
      }, "Shows inaccessible channels, restores real names when Discord still has them locally, and disables Discord's private-channel metadata obfuscation setting. The channel-mode patch is render-only and does not grant access to channel contents."),
