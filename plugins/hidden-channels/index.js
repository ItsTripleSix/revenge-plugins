(() => {
  ...
  const resolveChannel = value => {
    ...
    if (typeof value === "object") return value.channel ?? value.record ?? value;
    try { return channelStore()?.getChannel?.(value) ?? null; } catch { return null; }
  };
  ...
  const isGuildChannel = channel => !!channel && typeof channel.type === "number" && guildId(channel) != null;
  const isObfuscated = channel => {
    try { return channel?.isObfuscated?.() === true; } catch { return false; }
  };
  ...
  function lacksView(channel) {
    ...
    try { return rt.realCan(rt.viewChannel, channel) === false; } catch { return false; }
  }
  ...
  const rawName = value => {
    ...
    if (!trimmed || /^no[ -]?access$/i.test(trimmed)) return null;
    return trimmed;
  };

  const isNoAccess = value => typeof value === "string" && /^\s*no\s+access\s*$/i.test(value);

  function replaceNoAccess(node, name) {
    if (isNoAccess(node)) return name;
    if (Array.isArray(node)) {
      let changed = false;
      const next = node.map(child => {
        const value = replaceNoAccess(child, name);
        changed ||= value !== child;
        return value;
      });
    ...
    const nextChildren = children === undefined ? children : replaceNoAccess(children, name);
  ...
  function patchChannelList() {
    ...
    const a = safePatch("channelListGuild", () => after("getGuild", proto, (_args, result) => rt.active ? revealGuildList(result) : result));
    const b = safePatch("channelListRows", () => after("getGuildChannelRowsOnly", proto, (_args, result) => rt.active ? revealGuildList(result) : result));
  ...
  function patchChannelRows() {
    ...
    return safePatch("channelRows", () => after(method, target, (args, result) => {
      ...
      const channel = resolveChannel(args?.[0]?.channel);
      if (!isHidden(channel)) return result;
      const name = rawName(channel);
      return name ? replaceNoAccess(result, name) : result;
    }));
  }
  ...
  function patchNameHelpers() {
    ...
    if (typeof module.computeChannelName === "function") {
      changed = safePatch("computeChannelName", () => after("computeChannelName", module, (args, result) => {
        if (!rt.active || !isNoAccess(result)) return result;
        const channel = resolveChannel(args?.[0]);
        return isHidden(channel) ? (rawName(channel) ?? result) : result;
      })) || changed;
    }
  ...
  function Settings() {
    return React.createElement(
      ...
      }, "Render-only build modeled after Aliucord's approach. It reveals hidden rows and replaces Discord's No Access label with the channel object's raw name when available. It does not touch the gateway, sockets, reconnect logic, experiments, or Discord's cache."),
      React.createElement(RN.Pressable, {
        onPress: inspectRawNames,
        style: { minHeight: 46, borderRadius: 8, alignItems: "center", justifyContent: "center", backgroundColor: "#5865F2", paddingHorizontal: 14 },
      }, React.createElement(RN.Text, {
        style: { color: "#FFFFFF", fontSize: 15, fontWeight: "700" },
      }, "Inspect raw hidden names")),
      React.createElement(RN.Text, {
        style: { color: "#80848E", fontSize: 12, lineHeight: 17, marginTop: 10 },
      }, "If any channel still says No Access, tap the button and send me the three counts. That tells us whether the modern client still has the real names locally."),
    );
  }
  ...
  return {
    onLoad() {
      ...
      if (![...early, ...current].some(Boolean)) throw new Error("Discord channel-list modules were not found");
    },
    onUnload: cleanup,
    settings: Settings,
  };
