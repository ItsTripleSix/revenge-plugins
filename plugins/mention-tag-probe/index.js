(() => {
  ...
  function Settings() {
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Mention Tag Probe v5"),
        React.createElement(RN.Text, { style: text }, "This now controls the actual native @mention text. After changing a setting, leave the channel and return once so Discord rebuilds the native message row."),
      ),
      React.createElement(RN.View, { style: card },
  ...
  return {
    onLoad() {
      ...
      try { showToast?.(unpatch ? "Mention probe v5 loaded" : "Mention probe v5: AST hook unavailable"); } catch {}
    },
