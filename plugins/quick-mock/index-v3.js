(() => {
  ...
  function patchChatInput() {
    ...
    if (!ChatInput?.prototype?.render || !findInReactTree) return null;
      ...
      return after("render", ChatInput.prototype, (_args, result) => {
        try {
          const props = findInReactTree(
            ...
            node => node?.type?.name === "ChatInput" && typeof node?.props?.onChangeText === "function",
          )?.props;
  ...
  function setLiveComposerText(text) {
    try {
      if (typeof liveInputProps?.onChangeText === "function") {
    ...
    // Fallback for builds where ChatInput changed shape.
    try {
  ...
  function putInComposer(channelId, text) {
    // Updating the mounted input is the path that makes text appear immediately.
    const liveWorked = setLiveComposerText(text);
    const draftWorked = saveDraft(channelId, text);

    if (liveWorked || draftWorked) {
      if (!liveWorked) {
        // Give the input a second chance after the action sheet has fully closed.
        setTimeout(() => {
          if (!setLiveComposerText(text)) toast("Mock saved as draft");
        }, 180);
      }
      return true;
    }

    // Retry after sheet animation; some builds temporarily unmount the composer.
    setTimeout(() => {
      if (setLiveComposerText(text) || saveDraft(channelId, text)) return;
  ...
  function handleMessageMock(message) {
    try {
      ...
      // Delay until the action sheet starts closing; this keeps Discord from
      // swallowing the composer update during the sheet transition.
      setTimeout(() => putInComposer(channelId, mocked), 60);
    } catch (error) {
