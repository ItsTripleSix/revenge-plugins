(() => {
  ...
  const VERSION = "1.0.2";
  const SHORTCUT_KEY = "ITS_TRIPLE_SIX_ACCOUNT_SWITCHER";
  const PURGE_SHORTCUT_KEY = "ITS_TRIPLE_SIX_PURGE_TOOLS";
  ...
  function enableNativeSwitcher() {
    ...
    if (!runtime.nativeEnabled) {
      try {
        runtime.notificationHadOwn = Object.prototype.hasOwnProperty.call(
          runtime.multiAccountStore,
          "canUseMultiAccountNotifications",
        );
        runtime.notificationDescriptor = Object.getOwnPropertyDescriptor(
          runtime.multiAccountStore,
          "canUseMultiAccountNotifications",
        ) ?? null;

        Object.defineProperty(runtime.multiAccountStore, "canUseMultiAccountNotifications", {
          configurable: true,
          get: () => true,
        });
      } catch {}
  ...
  function currentId() {
    try { return String(runtime.userStore?.getCurrentUser?.()?.id ?? ""); } catch { return ""; }
  }
  ...
  function Settings() {
    ...
    const children = [
      React.createElement(RN.View, {
        ...
        }, available && runtime.nativeEnabled ? "Discord native multi-account enabled" : "Native multi-account module not found yet"),
      ]),
    ...
    try {
      runtime.settingsUnpatch = V.patcher.after("createList", createListModule, args => {
        try {
          ...
          const purgeIndex = section.settings.indexOf(PURGE_SHORTCUT_KEY);
          const pluginsIndex = section.settings.indexOf("BUNNY_PLUGINS");
          const insertAt = purgeIndex >= 0
            ? purgeIndex + 1
            : pluginsIndex >= 0
              ? pluginsIndex + 1
  ...
  function cleanup() {
    ...
    try {
      if (runtime.multiAccountStore) {
        if (runtime.notificationHadOwn && runtime.notificationDescriptor) {
          Object.defineProperty(
            runtime.multiAccountStore,
            "canUseMultiAccountNotifications",
            runtime.notificationDescriptor,
          );
        } else {
          delete runtime.multiAccountStore.canUseMultiAccountNotifications;
        }
      }
    } catch {}
