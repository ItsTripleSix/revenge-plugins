(() => {
  ...
  function ensureColorTimer() {
    if (colorTimer || !colorSubscribers.size || !appIsActive) return;
  ...
  function effectiveMentionConfig() {
    ...
    if (requested === "theme" && theme.hasTheme) {
      return { source: "theme", background: theme.background, line: theme.line };
    }
    return { source: "discord", background: null, line: null };
  }
    ...
    try {
      return after("generateMessageRowData", MessageRowGenerator, (args, result) => {
        try {
          ...
          const next = { ...existing };
          const background = nativeColor(cfg.background);
          const line = nativeColor(cfg.line);
          if (background != null) next.backgroundColor = background;
          if (line != null) next.gutterColor = line;
  ...
  function Settings() {
    ...
    function set(key, value) { storage[key] = value; forceUpdate(); refreshToolkitUI(); }
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.5.0"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text }, "Mentions are now wired into Toolkit. This first pass exposes the mentioned-message background and left gutter/side-line so we can verify the renderer path before adding animated effects."),
      ),
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Mention highlights"),
        React.createElement(RN.Text, { style: text }, "Controls the full-message highlight shown when a message mentions you. Theme / Auto follows theme metadata or semantic colors. Discord leaves the native appearance untouched."),
        React.createElement(Choice, {
