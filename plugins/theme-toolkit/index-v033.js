(() => {
  ...
  // Smooth chase: one native-driven orb plus a small number of trailing dots.
  // This intentionally does not animate dozens of perimeter points on the JS thread.
  ...
  function trailSpan(trail) {
    if (trail === "short") return 0.075;
    if (trail === "long") return 0.24;
    return 0.15;
  }
  ...
  function ChaseVisual({ outline, baseStyle, fallbackRadius }) {
    ...
    React.useEffect(() => {
      if (!phase || !Animated?.timing || !Animated?.loop) return undefined;
    ...
    const dot = Math.max(4, outline.width * 2.35);
    ...
    const baseOpacity = outline.brightness === "max" ? 0.12 : outline.brightness === "bright" ? 0.08 : 0.05;
    const headHalo = outline.brightness === "max" ? 7 : outline.brightness === "bright" ? 5 : 3.5;
    const Wrapper = Animated?.View ?? RN.View;

    const dots = [];
    if (size.width > 0 && size.height > 0 && phase) {
      for (let i = count - 1; i >= 0; i--) {
        const frac = count <= 1 ? 0 : i / (count - 1);
        const offset = frac * span;
        const motion = pathTransform(phase, size.width, size.height, radius, offset);
        const opacity = i === 0 ? 1 : Math.max(0.08, Math.pow(1 - frac, 1.45) * 0.78);
        const scale = i === 0 ? 1.5 : Math.max(0.48, 1 - frac * 0.5);
        const color = palette.length <= 1 ? baseColor : gradientAt(palette, frac * 0.9);
        const halo = i === 0 ? headHalo : Math.max(0.8, headHalo * (1 - frac) * 0.42);
  ...
  function Settings() {
    ...
    function OutlineCard({ stateName, prefix }) {
      ...
      return React.createElement(RN.View, { style: card },
        ...
        React.createElement(RN.Text, { style: text },
          "Chase = one smooth native-driven glowing orb with a fading trail. Marquee = moving bright border sections."
        ),
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.3.3"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text },
          "Chase was rebuilt to use a native-driven orb that follows the actual rounded perimeter instead of hopping between pre-rendered points."
        ),
      ),
