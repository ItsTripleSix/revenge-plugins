(() => {
  ...
  const batchProbe = {
    hits: {
      guildStyle: 0, guildDirect: 0, guildIndicator: 0, guildWrapper: 0, guildWrapperSelected: 0, guildWrapperUnread: 0, homeExport: 0, chatExport: 0, searchExport: 0, channelSearchExport: 0, notificationExport: 0, settingsExport: 0,
      baseIcon: 0, baseHome: 0, baseChat: 0, baseSearch: 0, baseChannelSearch: 0, baseNotification: 0, baseSettings: 0, searchButton: 0,
      legacyIcon: 0, legacyHome: 0, legacySearch: 0, legacyNotification: 0, legacySettings: 0, nativeImage: 0, nativeHome: 0, nativeSearch: 0, nativeNotification: 0, nativeSettings: 0,
      designIcon: 0, designSearch: 0, designNotification: 0, designSettings: 0, guildSearchParent: 0, guildSearchInner: 0, guildSearchButton: 0,
      guildRendererReplacement: 0, guildOverlayFallback: 0, iconAction: 0, iconActionSearch: 0, iconActionNotification: 0, iconActionSettings: 0, headerSearch: 0, headerNotification: 0, headerSettings: 0,
      messageRows: 0, reactionRows: 0, rowsWithReactions: 0,
      mentionedRows: 0, mentionBackgroundApplied: 0, mentionGutterApplied: 0,
      mentionParses: 0, mentionTagsTinted: 0,
    },
  };
  const iconAssetIds = (() => {
    try {
  ...
  function storedProfiles() {
    ...
    return storage.toolkitProfiles.slice(0, PROFILE_LIMIT).flatMap((profile, index) => {
      if (!profile || typeof profile !== "object" || typeof profile.name !== "string" || !profile.values || typeof profile.values !== "object") return [];
      ...
      return [{
        id: typeof profile.id === "string" && profile.id ? profile.id : `legacy-${index}`,
        name,
  ...
  function countRendererTint(prefix, kind) {
    if (prefix === "legacy") {
      batchProbe.hits.legacyIcon++;
      if (kind === "home") batchProbe.hits.legacyHome++;
      else if (kind === "search") batchProbe.hits.legacySearch++;
      else if (kind === "notification") batchProbe.hits.legacyNotification++;
      else if (kind === "settings") batchProbe.hits.legacySettings++;
    } else {
      batchProbe.hits.nativeImage++;
      if (kind === "home") batchProbe.hits.nativeHome++;
      else if (kind === "search") batchProbe.hits.nativeSearch++;
      else if (kind === "notification") batchProbe.hits.nativeNotification++;
      else if (kind === "settings") batchProbe.hits.nativeSettings++;
    }
  }
    ...
    try {
      return before(method, target, args => {
        try {
          ...
          countRendererTint("legacy", accent.kind);
          args[0] = {
            ...props,
            color,
            disableColor: false,
    ...
    try {
      return before(method, target, args => {
        try {
          ...
          const color = stripAlpha(accent?.color);
          if (!accent || !color) return;
          countRendererTint("native", accent.kind);
    ...
    try {
      return before(method, target, args => {
        try {
          ...
          if (!icon) return;
          if (accent.kind === "search") batchProbe.hits.designSearch++;
          else if (accent.kind === "notification") batchProbe.hits.designNotification++;
          else if (accent.kind === "settings") batchProbe.hits.designSettings++;
          args[0] = { ...props, icon };
        } catch (error) {
  ...
  function wrapSearchRendererElement(element) {
    ...
    if (!wrapper) {
      const name = iconComponentName(original);
      wrapper = function ThemeToolkitSearchRenderer(props) {
        if (/SearchButtonContent/i.test(name)) batchProbe.hits.guildSearchButton++;
        else batchProbe.hits.guildSearchInner++;
  ...
  function patchGuildSearchContainerRenderer() {
    ...
    try {
      return after(method, target, (_args, result) => {
        try {
          batchProbe.hits.guildSearchParent++;
          const cfg = effectiveUIAccentConfig();
          const color = cfg.source === "discord" ? null : stripAlpha(cfg.searchIcon);
    ...
    try {
      return after("default", IconActionButtonModule, (args, result) => {
        try {
          ...
          if (kind === "search") batchProbe.hits.iconActionSearch++;
          else if (kind === "notification") batchProbe.hits.iconActionNotification++;
          else batchProbe.hits.iconActionSettings++;
      ...
      return after("HeaderIcon", LegacyHeaderIconModule, (args, result) => {
        try {
          ...
          if (isNotificationSource(source)) {
            ...
            if (color) {
              batchProbe.hits.headerNotification++;
      ...
      return before("HeaderIconButton", HeaderSharedModule, args => {
        try {
          ...
          if (isSearchSource(props.source)) {
            ...
            const color = stripAlpha(cfg.notificationIcon);
            if (color) { batchProbe.hits.headerNotification++; props.color = color; props.tintColor = color; }
    ...
    try {
      return after("default", GuildBarWrapperModule, (args, result) => {
        try {
          ...
          const props = args?.[0] ?? {};
          if (props.selected) batchProbe.hits.guildWrapperSelected++;
          if (props.unread && !props.selected) batchProbe.hits.guildWrapperUnread++;
          ...
          for (let rootIndex = 0; rootIndex < rootChildren.length; rootIndex++) {
            ...
            batchProbe.hits.guildRendererReplacement++;
          ...
          batchProbe.hits.guildOverlayFallback++;
          return React.cloneElement(result, null, ...rootChildren);
        } catch (error) {
    ...
    try {
      return before(method, module, args => {
        try {
          ...
          else if (["BellIcon", "BellSlashIcon", "BellZIcon", "ChannelNotificationIcon", "ChatNotificationIcon"].includes(method)) batchProbe.hits.notificationExport++;
      ...
      return before("BaseIconImage", BaseIconImageModule, args => {
        try {
          ...
          if (iconSourceMatches(props.source, iconAssetIds.home, "HomeIcon")) {
            ...
            batchProbe.hits.baseNotification++;
  ...
  function patchMentionTags() {
    ...
    try {
      return after("parseMessageMarkup", MarkupParsers, (_args, result) => {
        try {
          batchProbe.hits.mentionParses++;
          batchProbe.hits.mentionTagsTinted += tintMentionTags(result?.content);
        } catch (error) {
      ...
      return after("generateMessageRowData", MessageRowGenerator, (args, result) => {
        try {
          ...
          if (cfg.backgroundEnabled && background != null) {
            ...
            batchProbe.hits.mentionBackgroundApplied++;
          }
          if (cfg.lineEnabled && line != null) {
            next.gutterColor = line;
            batchProbe.hits.mentionGutterApplied++;
          }
  ...
  function Settings() {
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.7.4 TEST"),
      ...
      React.createElement(RN.View, { style: card },
        ...
        React.createElement(RN.Text, { style: text }, `Legacy/native fallbacks: ${componentRenderTarget(LegacyIconModule?.default) ? "FOUND" : "MISSING"}/${componentRenderTarget(RN.Image) ? "FOUND" : "MISSING"}`),
        React.createElement(RN.Text, { style: text }, `Design IconButton: ${componentRenderTarget(DesignIconButtonModule?.IconButton) ? "FOUND" : "MISSING"} • total ${batchProbe.hits.designIcon} • Search ${batchProbe.hits.designSearch} • Bell ${batchProbe.hits.designNotification} • Settings ${batchProbe.hits.designSettings}`),
        React.createElement(RN.Text, { style: text }, `Server Search parent: ${componentRenderTarget(GuildSearchContainerModule?.default) ? "FOUND" : "MISSING"} • parent ${batchProbe.hits.guildSearchParent} • inner ${batchProbe.hits.guildSearchInner} • button ${batchProbe.hits.guildSearchButton}`),
        ...
        React.createElement(RN.Text, { style: text }, `Bell/notification export/base hits: ${batchProbe.hits.notificationExport}/${batchProbe.hits.baseNotification}`),
        ...
        React.createElement(RN.Text, { style: text }, `Shared action icons: ${batchProbe.hits.iconAction} • Search ${batchProbe.hits.iconActionSearch} • Bell ${batchProbe.hits.iconActionNotification} • Settings ${batchProbe.hits.iconActionSettings}`),
        React.createElement(RN.Text, { style: text }, `Header source hits: Search ${batchProbe.hits.headerSearch} • Bell ${batchProbe.hits.headerNotification} • Settings ${batchProbe.hits.headerSettings}`),
        React.createElement(RN.Text, { style: text }, `Legacy assets: Home ${batchProbe.hits.legacyHome} • Search ${batchProbe.hits.legacySearch} • Bell ${batchProbe.hits.legacyNotification} • Settings ${batchProbe.hits.legacySettings}`),
        React.createElement(RN.Text, { style: text }, `Native assets: Home ${batchProbe.hits.nativeHome} • Search ${batchProbe.hits.nativeSearch} • Bell ${batchProbe.hits.nativeNotification} • Settings ${batchProbe.hits.nativeSettings}`),
        React.createElement(RN.Text, { style: text }, `Guild indicators: wrappers ${batchProbe.hits.guildWrapper} • selected ${batchProbe.hits.guildWrapperSelected} • unread ${batchProbe.hits.guildWrapperUnread} • renderer replaced ${batchProbe.hits.guildRendererReplacement} • fallback ${batchProbe.hits.guildOverlayFallback}`),
        ...
        React.createElement(RN.Text, { style: text }, `Mentioned rows: ${batchProbe.hits.mentionedRows} • background applied ${batchProbe.hits.mentionBackgroundApplied} • gutter applied ${batchProbe.hits.mentionGutterApplied}`),
        React.createElement(RN.Text, { style: text }, `Mention parser: ${MarkupParsers?.parseMessageMarkup ? "FOUND" : "MISSING"} • parses ${batchProbe.hits.mentionParses} • tags tinted ${batchProbe.hits.mentionTagsTinted}`),
      ),
      React.createElement(RN.View, { style: card },
