(() => {
  ...
  function trailCount(trail) {
    if (trail === "short") return 16;
    if (trail === "long") return 28;
    return 22;
  }
  function trailSpan(trail) {
    if (trail === "short") return 0.58;
    if (trail === "long") return 0.96;
    return 0.78;
  }
  ...
  function ChaseVisual({ outline, baseStyle, fallbackRadius }) {
    ...
    if (size.width > 0 && size.height > 0 && motionPhase) {
      for (let i = count - 1; i >= 0; i--) {
        ...
        const scale = i === 0 ? 1.58 : Math.max(0.42, 1 - frac * 0.55);
  ...
  function Settings() {
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.3.5"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text }, "Chase trail lengths were greatly extended. Chase colors now use a real timed color update instead of Animated color interpolation, so RGB/Rainbow/Full Spectrum/Gradient visibly cycle on Android."),
      ),
      React.createElement(RN.View, { style: card },
