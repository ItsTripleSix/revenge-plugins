(() => {
  ...
  const batchProbe = {
    hits: {
      ...
      guildOverlayHost: 0, guildOverlayFallback: 0, iconAction: 0, iconActionSearch: 0, iconActionSettings: 0, headerSearch: 0, headerSettings: 0,
  ...
  function selectedGuildOverlay(color, key, top) {
    return React.createElement(RN.View, {
      key,
      pointerEvents: "none",
      style: {
        position: "absolute", left: -4, top, width: 8, height: 40,
        borderRadius: 4, backgroundColor: color, zIndex: 999, elevation: 999,
      },
    });
  }
      ...
      return after("default", GuildBarWrapperModule, (args, result) => {
        try {
          ...
          for (let rootIndex = 0; rootIndex < rootChildren.length; rootIndex++) {
            ...
            hostChildren.push(selectedGuildOverlay(color, "tt-selected-guild-host-overlay", 8));
            ...
            batchProbe.hits.guildOverlayHost++;
            return React.cloneElement(result, null, ...rootChildren);
          }

          rootChildren.push(selectedGuildOverlay(color, "tt-selected-guild-fallback-overlay", 14));
          batchProbe.hits.guildOverlayFallback++;
          return React.cloneElement(result, null, ...rootChildren);
        } catch (error) {
          try { console.error("[ThemeToolkit] selected guild host overlay failed", error); } catch {}
          return result;
        }
      });
    } catch (error) {
  ...
  function Settings() {
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.6.7 TEST"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text }, "Batch verification: live icon paths, the selected-indicator host, and both current and legacy mentioned-message theme tokens are handled directly."),
      ),
      React.createElement(RN.View, { style: card },
        ...
        React.createElement(RN.Text, { style: text }, `Guild wrapper: hits ${batchProbe.hits.guildWrapper} • selected ${batchProbe.hits.guildWrapperSelected} • host ${batchProbe.hits.guildOverlayHost} • fallback ${batchProbe.hits.guildOverlayFallback}`),
