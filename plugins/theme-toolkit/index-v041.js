(() => {
  ...
  function ChaseVisual({ outline, baseStyle, fallbackRadius }) {
    ...
    const compactThickness = Math.max(2.6, outline.width * 1.55);
    const solidLength = compactSolid
      ? Math.max(compactThickness * 1.45, pathSpacing * 1.72)
    ...
    if (size.width > 0 && size.height > 0 && motionPhase) {
      for (let i = count - 1; i >= 0; i--) {
        ...
        const minScale = compactSolid ? 0.88 : outline.pattern === "solid" ? 0.74 : 0.42;
        const scale = i === 0 ? 1.58 : Math.max(minScale, 1 - frac * (compactSolid ? 0.12 : outline.pattern === "solid" ? 0.26 : 0.55));
  ...
  function Settings() {
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.4.1"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text }, "Background pause/resume remains enabled. Collapsed Solid Chase now uses thinner, longer overlapping trail capsules with trail halos suppressed, so the small rounded folder should read as one continuous fading line instead of a chain of dots."),
      ),
      React.createElement(RN.View, { style: card },
