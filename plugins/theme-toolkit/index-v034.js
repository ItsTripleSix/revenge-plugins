(() => {
  ...
  function trailCount(trail) {
    if (trail === "short") return 8;
    if (trail === "long") return 19;
    return 13;
  }
  function trailSpan(trail) {
    if (trail === "short") return 0.22;
    if (trail === "long") return 0.58;
    return 0.38;
  }
  ...
  function ChaseVisual({ outline, baseStyle, fallbackRadius }) {
    ...
    if (!colorRef.current && Animated?.Value) colorRef.current = new Animated.Value(0);
    const colorPhase = colorRef.current;
    ...
    React.useEffect(() => {
      ...
      return () => {
        try { loop.stop?.(); } catch {}
        try { motionPhase.stopAnimation?.(); } catch {}
      };
    }, [outline.speed]);

    React.useEffect(() => {
      if (!dynamicColor || !colorPhase || !Animated?.timing || !Animated?.loop) return undefined;
      try { colorPhase.stopAnimation?.(); colorPhase.setValue?.(0); } catch {}
      const loop = Animated.loop(Animated.timing(colorPhase, {
        toValue: 1,
        duration: Math.round(speedMs(outline.speed) * 1.35),
        easing: Easing?.linear,
      ...
      return () => {
        try { loop.stop?.(); } catch {}
        try { colorPhase.stopAnimation?.(); } catch {}
      };
    }, [dynamicColor, outline.speed, paletteKey]);

    const radius = Math.max(
      0,
      Math.min(
        radiusFor(baseStyle, fallbackRadius),
        size.width / 2 || fallbackRadius,
        size.height / 2 || fallbackRadius,
      ),
    );
    const dot = Math.max(4.4, outline.width * 2.55);
    const count = trailCount(outline.trail);
    const span = trailSpan(outline.trail);
    const baseColor = palette[0] ?? "#FFFFFF";
    const chaseColor = dynamicColor ? animatedCycleColor(colorPhase, palette, 0) : baseColor;
    const baseOpacity = outline.brightness === "max" ? 0.14 : outline.brightness === "bright" ? 0.09 : 0.05;
    const headHalo = outline.brightness === "max" ? 7.5 : outline.brightness === "bright" ? 5.5 : 3.5;
    ...
    if (size.width > 0 && size.height > 0 && motionPhase) {
      for (let i = count - 1; i >= 0; i--) {
        ...
        const motion = pathTransform(motionPhase, size.width, size.height, radius, offset);
        const opacity = i === 0
          ? 1
          : Math.max(0.14, Math.pow(1 - frac, 1.1) * 0.9);
        const scale = i === 0 ? 1.55 : Math.max(0.5, 1 - frac * 0.48);
        const halo = i === 0 ? headHalo : Math.max(0.75, headHalo * (1 - frac) * 0.46);
        const haloOpacity = i === 0 ? 0.5 : Math.max(0.05, 0.19 * (1 - frac));

        dots.push(React.createElement(Wrapper, {
          key: `tt-chase-dot-${i}`,
          pointerEvents: "none",
          style: {
            position: "absolute",
          ...
          React.createElement(Wrapper, {
            ...
            style: {
              ...
              right: -halo,
              bottom: -halo,
              borderRadius: dot + halo * 2,
              backgroundColor: chaseColor,
              opacity: haloOpacity,
            },
          }),
          i === 0 ? React.createElement(RN.View, {
            pointerEvents: "none",
            style: {
              position: "absolute",
              top: dot * 0.4,
    ...
    return React.createElement(RN.View, {
      ...
      onLayout(event) {
        ...
        if (
          width > 0
          && height > 0
          && (Math.abs(width - size.width) > 0.5 || Math.abs(height - size.height) > 0.5)
        ) {
  ...
  function OutlineVisual({ folder, state, baseStyle, fallbackRadius }) {
    ...
    if (outline.animation === "chase") {
      return React.createElement(ChaseVisual, { outline, baseStyle, fallbackRadius });
    }
    ...
    const needsMotion = dynamicColor
  ...
  function Settings() {
    ...
    function OutlineCard({ stateName, prefix }) {
      ...
      return React.createElement(RN.View, { style: card },
        ...
        React.createElement(RN.Text, { style: text },
          "RGB = red/green/blue. Rainbow = many rainbow hues. Full spectrum = a vivid sweep through the complete hue wheel."
        ),
        ...
        React.createElement(RN.Text, { style: text },
          "Chase = one smooth glowing orb with a fading trail. Its movement stays native-driven; only the color cycle uses a lightweight color animation."
        ),
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.3.4"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text },
          "Chase trails are much longer and more visible, while RGB/Rainbow/Full spectrum/Gradient now cycle the orb and its entire trail."
        ),
      ),
