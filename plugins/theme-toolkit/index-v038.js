(() => {
  ...
  function getSharedMotionPhase(speed) {
    ...
    const value = new Animated.Value(start);
    const entry = { value, first: null, loop: null, stopped: false };
    motionClocks.set(key, entry);
    const options = { easing: Easing?.linear, useNativeDriver: true, isInteraction: false };

    const beginLoop = () => {
      if (entry.stopped) return;
      try { value.setValue?.(0); } catch {}
      const loop = Animated.loop(Animated.timing(value, { toValue: 1, duration, ...options }));
      entry.loop = loop;
      try { loop.start(); } catch {}
    };

    const remaining = Math.max(1, Math.round(duration * (1 - start)));
    const first = Animated.timing(value, { toValue: 1, duration: remaining, ...options });
    entry.first = first;
    try {
      first.start(result => {
        if (entry.stopped) return;
  ...
  function pathGeometry(width, height, radius, offset) {
    ...
    const geometry = { inputRange: PATH_INPUT_RANGE, x: points.map(p => p.x), y: points.map(p => p.y) };
  ...
  function ChaseDot({ motionPhase, width, height, radius, offset, dot, opacity, scale, color, halo, haloOpacity, head }) {
    ...
    return React.createElement(Wrapper, {
      ...
      haloOpacity > 0 ? React.createElement(RN.View, {
        pointerEvents: "none",
        style: {
  ...
  function ChaseVisual({ outline, baseStyle, fallbackRadius }) {
    ...
    if (size.width > 0 && size.height > 0 && motionPhase) {
      for (let i = count - 1; i >= 0; i--) {
        ...
        const scale = i === 0 ? 1.58 : Math.max(minScale, 1 - frac * (outline.pattern === "solid" ? 0.26 : 0.55));
        const halo = i === 0 ? headHalo : Math.max(0.7, headHalo * (1 - frac) * 0.46);
        const haloOpacity = i === 0 ? 0.54 : (frac < 0.74 ? Math.max(0.04, 0.20 * (1 - frac)) : 0);
  ...
  function Settings() {
    ...
    function OutlineCard({ stateName, prefix }) {
      ...
      return React.createElement(RN.View, { style: card },
        ...
        React.createElement(RN.Text, { style: text }, "Chase = one smooth glowing orb with a long fading tail. Marquee = moving bright border sections."),
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.3.8"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text }, "Animation phases are shared across folders, so opening/closing no longer restarts at red or phase zero. Chase path geometry is cached and lighter to reduce opening lag, and Marquee now follows the perimeter in true order with smoother motion."),
      ),
      React.createElement(RN.View, { style: card },
