(() => {
  ...
  const APPEARANCE_DEFAULTS = {
    ...
    // Visual test defaults stay deliberately loud until the final release cleanup.
  ...
  const batchProbe = {
    hits: {
      guildStyle: 0, guildDirect: 0, guildIndicator: 0, guildWrapper: 0, guildWrapperSelected: 0, guildWrapperUnread: 0, homeExport: 0, chatExport: 0, searchExport: 0, channelSearchExport: 0, settingsExport: 0,
      baseIcon: 0, baseHome: 0, baseChat: 0, baseSearch: 0, baseChannelSearch: 0, baseSettings: 0, searchButton: 0,
      legacyIcon: 0, legacyHome: 0, legacySearch: 0, legacySettings: 0, nativeImage: 0, nativeHome: 0, nativeSearch: 0, nativeSettings: 0,
      designIcon: 0, designSearch: 0, designSettings: 0, guildSearchParent: 0, guildSearchInner: 0, guildSearchButton: 0,
      guildRendererReplacement: 0, guildOverlayFallback: 0, iconAction: 0, iconActionSearch: 0, iconActionSettings: 0, headerSearch: 0, headerSettings: 0,
  ...
  function applyAppearanceValues(values) {
    ...
    for (const key of PROFILE_SETTING_KEYS) {
      ...
      if (value == null || typeof value === "string" || typeof value === "number" || typeof value === "boolean") storage[key] = value;
    }
  }
    ...
    try {
      return before(method, target, args => {
        try {
          ...
          const color = stripAlpha(accent?.color);
          if (!accent || !["search", "settings"].includes(accent.kind) || !color) return;
    ...
    try {
      return after("default", IconActionButtonModule, (args, result) => {
        try {
          ...
          const color = stripAlpha(kind === "search" ? cfg.searchIcon : cfg.settingsIcon);
  ...
  function Settings() {
    ...
    function loadProfile(profile) {
      ...
      toast(`Loaded profile: ${profile.name}`);
    }
    function applyPalette() {
      ...
      applyValues(values);
      toast(storage.autoPaletteScope === "all" ? "Applied palette to the whole Toolkit" : "Applied palette to UI accents");
    }
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.7.0 TEST"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text }, "New batch: generate a coordinated palette from one color, save the complete Toolkit appearance as a named profile, then switch palettes and restore the saved profile."),
      ),
      React.createElement(RN.View, { style: card },
        ...
        React.createElement(RN.Text, { style: text }, storage.autoPaletteScope === "all"
          ...
          : "UI accents only changes server indicators, reactions, Home, Search, and Settings colors."),
        React.createElement(ActionButton, { labelText: "Apply palette", onPress: applyPalette, disabled: !autoPalette }),
      ),
      React.createElement(RN.View, { style: card },
        ...
        profiles.length ? React.createElement(RN.View, { style: { gap: 10 } }, profiles.map(profile =>
          React.createElement(RN.View, {
            ...
            React.createElement(RN.Text, { style: label }, profile.name),
      ...
      React.createElement(RN.View, { style: card },
        ...
        React.createElement(RN.Text, { style: text }, `Design IconButton: ${componentRenderTarget(DesignIconButtonModule?.IconButton) ? "FOUND" : "MISSING"} • total ${batchProbe.hits.designIcon} • Search ${batchProbe.hits.designSearch} • Settings ${batchProbe.hits.designSettings}`),
        ...
        React.createElement(RN.Text, { style: text }, `Legacy assets: Home ${batchProbe.hits.legacyHome} • Search ${batchProbe.hits.legacySearch} • Settings ${batchProbe.hits.legacySettings}`),
        React.createElement(RN.Text, { style: text }, `Native assets: Home ${batchProbe.hits.nativeHome} • Search ${batchProbe.hits.nativeSearch} • Settings ${batchProbe.hits.nativeSettings}`),
      ...
      React.createElement(RN.View, { style: card },
        ...
        React.createElement(RN.Text, { style: text }, "Toolkit test defaults are deliberately obvious: server indicators = magenta, reacted reaction = lime, Home = cyan, Search = yellow, Settings = orange."),
        React.createElement(Choice, {
