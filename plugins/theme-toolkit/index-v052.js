(() => {
  ...
  const DEFAULTS = {
    ...
    mentionColorMode: "custom",
    mentionBackground: "",
    mentionLine: "",
    mentionGradient1: "#FF0000",
    mentionGradient2: "#00FFFF",
    mentionGradient3: "#FF00FF",
    mentionBackgroundEnabled: true,
    mentionLineEnabled: true,
    mentionBackgroundStrength: "medium",
    mentionEffect: "none",
    mentionSpeed: "normal",
  };
  ...
  function mentionRecentlyVisible(now = Date.now()) {
    return mentionAnimationEnabled && now - lastMentionRenderAt < 900;
  }
  function colorTimerNeeded(now = Date.now()) {
    return colorSubscribers.size > 0 || mentionRecentlyVisible(now);
  }
  function ensureColorTimer() {
    ...
    colorTimer = setInterval(() => {
      ...
      if (mentionRecentlyVisible(now)) refreshMentionUI();
      else stopColorTimerIfIdle();
    }, 150);
  }
  ...
  function normalizeMentionMode(value, fallback = "custom") {
    return ["custom", "rgb", "rainbow", "spectrum", "gradient"].includes(value) ? value : fallback;
  }
  function normalizeMentionEffect(value, fallback = "none") {
    return ["none", "glow", "pulse", "breathe", "flash", "flow"].includes(value) ? value : fallback;
  }
  function normalizeMentionStrength(value, fallback = "medium") {
    return ["subtle", "medium", "strong", "max"].includes(value) ? value : fallback;
  }
  ...
  function themeMentionConfig() {
    const theme = currentTheme();
    const data = theme?.data ?? null;
    if (!data) return { hasTheme: false, background: null, line: null };
    const semantic = data.semanticColors ?? {};
    const raw = data.rawColors ?? {};
  ...
  function mentionGradient() {
    return [
      colorValue(storage.mentionGradient1),
      colorValue(storage.mentionGradient2),
      colorValue(storage.mentionGradient3),
    ].filter(Boolean);
  }
  function effectiveMentionConfig() {
    ...
    const common = {
      effect: normalizeMentionEffect(storage.mentionEffect, "none"),
      speed: normalizeSpeed(storage.mentionSpeed, "normal"),
      backgroundEnabled: storage.mentionBackgroundEnabled !== false,
      lineEnabled: storage.mentionLineEnabled !== false,
      backgroundStrength: normalizeMentionStrength(storage.mentionBackgroundStrength, "medium"),
    };
    if (requested === "toolkit") {
      const colorMode = normalizeMentionMode(storage.mentionColorMode, "custom");
      const gradient = mentionGradient();
      return {
        ...common,
        source: "toolkit",
        colorMode,
  ...
  function mentionStrengthAlpha(value) {
    if (value === "subtle") return 0.10;
    if (value === "strong") return 0.28;
    if (value === "max") return 0.40;
    return 0.18;
  }
  function mentionShouldAnimate(cfg = effectiveMentionConfig()) {
    if (cfg.source === "discord") return false;
    const dynamicColor = ["rgb", "rainbow", "spectrum", "gradient"].includes(cfg.colorMode);
    return dynamicColor || cfg.effect !== "none";
  }
  function syncMentionAnimationState() {
    mentionAnimationEnabled = mentionShouldAnimate();
    if (!mentionAnimationEnabled) lastMentionRenderAt = 0;
    ensureColorTimer();
    stopColorTimerIfIdle();
  }
  function noteMentionRender() {
    lastMentionRenderAt = Date.now();
    if (mentionAnimationEnabled) ensureColorTimer();
  }
  ...
  function mentionPalette(cfg) {
    if (cfg.colorMode === "rgb") return RGB_RING.slice(0, -1);
    if (cfg.colorMode === "rainbow") return RAINBOW_RING.slice(0, -1);
    if (cfg.colorMode === "spectrum") return FULL_SPECTRUM.slice(0, -1);
    if (cfg.colorMode === "gradient" && cfg.gradient.length >= 2) return cfg.gradient.map(stripAlpha).filter(Boolean);
    if (cfg.effect === "flow") {
      const custom = [stripAlpha(cfg.line), stripAlpha(cfg.background)].filter(Boolean);
      if (custom.length >= 2) return custom;
    }
    return [];
  }
  function mentionEffectFactor(cfg, now) {
    const basePhase = sharedPhaseAt(cfg.speed, now);
    const wave = (Math.sin(basePhase * Math.PI * 2 - Math.PI / 2) + 1) / 2;
    if (cfg.effect === "glow") return 0.72 + wave * 0.28;
    if (cfg.effect === "pulse") return 0.42 + wave * 0.58;
    if (cfg.effect === "breathe") {
      const slow = sharedPhaseAt(cfg.speed, animationEpoch + (now - animationEpoch) * 0.62);
      const slowWave = (Math.sin(slow * Math.PI * 2 - Math.PI / 2) + 1) / 2;
      return 0.50 + slowWave * 0.50;
    }
    if (cfg.effect === "flash") {
      const period = Math.max(2500, speedMs(cfg.speed));
      const phase = (((now - animationEpoch) % period) + period) % period / period;
      const distance = Math.min(phase, 1 - phase);
      const x = Math.max(0, 1 - distance / 0.16);
      const smooth = x * x * (3 - 2 * x);
      return 0.40 + smooth * 0.60;
    }
    return 1;
  }
  function mentionVisualColors(cfg, now = Date.now()) {
    let line = cfg.line;
    let background = cfg.background;
    const palette = mentionPalette(cfg);
    if (palette.length >= 2) {
      const phase = sharedPhaseAt(cfg.speed, now);
      const linePhase = cfg.effect === "flow" ? phase * 1.35 : phase;
      line = gradientAt(palette, linePhase);
      const bgPhase = cfg.effect === "flow" ? linePhase + 0.16 : linePhase;
      background = withAlpha(gradientAt(palette, bgPhase), mentionStrengthAlpha(cfg.backgroundStrength));
    }
    const factor = mentionEffectFactor(cfg, now);
    if (line) line = multiplyAlpha(line, factor, 1);
    if (background) background = multiplyAlpha(background, cfg.effect === "none" || cfg.effect === "flow" ? 1 : 0.72 + factor * 0.28, mentionStrengthAlpha(cfg.backgroundStrength));
  ...
  function patchMentionRowRefresh() {
    const ChatManager = ChatManagerModule?.default;
    const proto = ChatManager?.prototype;
    if (!proto || typeof proto.determineChangeType !== "function" || typeof before !== "function") return null;
    try {
      return before("determineChangeType", proto, args => {
        try {
          const input = args?.[0];
          if (!mentionAnimationEnabled || !input?.message?.mentioned) return;
          input.forceRender = true;
        } catch {}
      });
    } catch (error) { try { console.error("[ThemeToolkit] failed to patch mention row refresh", error); } catch {} return null; }
  }
    ...
    try {
      return after("generateMessageRowData", MessageRowGenerator, (args, result) => {
        try {
          ...
          noteMentionRender();
          const visual = mentionVisualColors(cfg, Date.now());
          const existing = result.backgroundHighlight ?? {};
          const next = { ...existing };
          const background = cfg.backgroundEnabled ? nativeColor(visual.background) : null;
          const line = cfg.lineEnabled ? nativeColor(visual.line) : null;
        ...
        } catch (error) { try { console.error("[ThemeToolkit] mention highlight patch failed", error); } catch {} }
        return result;
      });
    } catch (error) { try { console.error("[ThemeToolkit] failed to patch mention highlights", error); } catch {} return null; }
  }
  ...
  function Settings() {
    ...
    const mentionDynamicMode = ["rgb", "rainbow", "spectrum", "gradient"].includes(storage.mentionColorMode);
    const mentionAnimated = mentionDynamicMode || storage.mentionEffect !== "none";
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.5.2"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text }, "Animated mention highlights now force only mentioned message rows to refresh while visible, bypassing Discord's cached row data. The timer still idles automatically when no mention is visible."),
      ),
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Mention highlights"),
        React.createElement(RN.Text, { style: text }, "Controls the full-message highlight shown when a message mentions you. Discord leaves the native appearance untouched."),
        React.createElement(Choice, {
        ...
        storage.mentionColorSource !== "discord" ? React.createElement(RN.View, { style: { gap: 10 } },
          storage.mentionColorSource === "toolkit" ? React.createElement(React.Fragment, null,
            React.createElement(RN.Text, { style: label }, "Color mode"),
            React.createElement(Choice, {
              value: storage.mentionColorMode,
              options: [
                { value: "custom", label: "Custom" }, { value: "rgb", label: "RGB" },
                { value: "rainbow", label: "Rainbow" }, { value: "spectrum", label: "Full spectrum" },
                { value: "gradient", label: "Gradient" },
              ],
              onChange: value => set("mentionColorMode", value),
            }),
            storage.mentionColorMode === "custom" ? React.createElement(RN.View, { style: { gap: 8 } },
            ...
            storage.mentionColorMode === "gradient" ? React.createElement(RN.View, { style: { gap: 8 } },
              React.createElement(ColorInput, { labelText: "Gradient color 1", storageKey: "mentionGradient1" }),
              React.createElement(ColorInput, { labelText: "Gradient color 2", storageKey: "mentionGradient2" }),
              React.createElement(ColorInput, { labelText: "Gradient color 3", storageKey: "mentionGradient3" }),
              React.createElement(RN.Text, { style: text }, "Message highlights cannot draw a spatial gradient in Discord's native row data, so Gradient smoothly cycles through your chosen colors."),
            ) : null,
          ) : null,
          ...
          storage.mentionColorSource === "toolkit" && storage.mentionColorMode !== "custom" ? React.createElement(React.Fragment, null,
            React.createElement(RN.Text, { style: label }, "Background strength"),
            React.createElement(Choice, {
              value: storage.mentionBackgroundStrength,
              options: [
                { value: "subtle", label: "Subtle" }, { value: "medium", label: "Medium" },
                { value: "strong", label: "Strong" }, { value: "max", label: "Max" },
              ],
              onChange: value => set("mentionBackgroundStrength", value),
            }),
          ) : null,
          React.createElement(RN.Text, { style: label }, "Effect"),
          React.createElement(Choice, {
            value: storage.mentionEffect,
            options: [
              { value: "none", label: "None" }, { value: "glow", label: "Glow" },
              { value: "pulse", label: "Pulse" }, { value: "breathe", label: "Breathe" },
              { value: "flash", label: "Flash" }, { value: "flow", label: "Color flow" },
            ],
            onChange: value => set("mentionEffect", value),
          }),
          React.createElement(RN.Text, { style: text }, "Glow, Pulse and Breathe vary intensity. Flash gives one short smooth attention peak per cycle rather than a rapid strobe. Color flow offsets the side line and background through the selected palette."),
          mentionAnimated ? React.createElement(React.Fragment, null,
            React.createElement(RN.Text, { style: label }, "Speed"),
            React.createElement(Choice, {
              value: storage.mentionSpeed,
              options: [{ value: "slow", label: "Slow" }, { value: "normal", label: "Normal" }, { value: "fast", label: "Fast" }],
              onChange: value => set("mentionSpeed", value),
            }),
          ) : null,
        ) : null,
      ),
  ...
  return {
    onLoad() {
      ...
      unpatchMentionRowRefresh = patchMentionRowRefresh();
    ...
    onUnload() {
      ...
      try { unpatchMentionRowRefresh?.(); } catch {}
      unpatchFolder = null;
      unpatchFolderBG = null;
      unpatchMentions = null;
      unpatchMentionRowRefresh = null;
      mentionAnimationEnabled = false;
      lastMentionRenderAt = 0;
