(() => {
  ...
  const GuildFolderModule = (() => {
    try { return findByProps("GuildsBarGuildFolderBG"); } catch { return null; }
  })();
  const FolderStore = (() => {
    try { return findByProps("getGuildFolders", "getGuildFolderById"); } catch { return null; }
  })();
  const FolderActions = (() => {
    try { return findByProps("saveGuildFolders"); } catch { return null; }
  })();
  const ColorUtils = (() => {
    try { return findByProps("int2hex", "hex2int"); } catch { return null; }
  })();
  const toolkitIcon = (() => {
    try {
  ...
  function notifyVisuals() {
    for (const fn of [...visualSubscribers]) {
      try { fn(); } catch {}
    }
  }
  ...
  function effectiveOutline(themeOutline, state, hasTheme) {
    ...
    const requestedBrightness = storage[`${prefix}OutlineBrightness`];
    const requestedTrail = storage[`${prefix}OutlineTrail`];
    ...
    return {
      ...
      brightness: requestedBrightness === "theme"
        ? (hasTheme ? themeOutline.brightness : "bright")
        : normalizeBrightness(requestedBrightness, hasTheme ? themeOutline.brightness : "bright"),
      trail: requestedTrail === "theme"
        ? (hasTheme ? themeOutline.trail : "medium")
        : normalizeTrail(requestedTrail, hasTheme ? themeOutline.trail : "medium"),
      state,
    };
  }
  ...
  function flattened(style) {
    try { return RN.StyleSheet?.flatten?.(style) ?? {}; } catch { return {}; }
  }
  ...
  function trailFraction(trail) {
    if (trail === "short") return 0.12;
    if (trail === "long") return 0.36;
    return 0.23;
  }
  ...
  const RAINBOW_RING = [
    ...
    "#0080FF", "#4B00FF", "#A000FF", "#FF00FF", "#FF0066", "#FF0000",
  ];
  const FULL_SPECTRUM = [
    "#FF0000", "#FF4000", "#FF8000", "#FFC000", "#FFFF00",
    "#BFFF00", "#80FF00", "#40FF00", "#00FF00", "#00FF40",
    "#00FF80", "#00FFC0", "#00FFFF", "#00BFFF", "#0080FF",
  ...
  function animatedCycleColor(phase, colors, offset = 0) {
    ...
    try { return phase.interpolate({ inputRange, outputRange }); }
    catch { return outputRange[0]; }
  }
  ...
  function marqueeOpacity(phase, index, count) {
    ...
    const points = [0, Math.max(0, center - half), center, Math.min(1, center + half), 1]
      .filter((v, i, arr) => i === 0 || v > arr[i - 1]);
    const values = points.map(v => Math.abs(v - center) < 0.0001 ? 1 : 0.12);
    try { return phase.interpolate({ inputRange: points, outputRange: values, extrapolate: "clamp" }); }
    catch { return 1; }
  }

  function chasePoints(width, baseStyle) {
    const flat = flattened(baseStyle);
    const boxW = Number(flat.width);
    const boxH = Number(flat.height);
    const hCount = 12;
    const ratio = Number.isFinite(boxW) && boxW > 0 && Number.isFinite(boxH) && boxH > 0
      ? Math.max(0.7, Math.min(2.4, boxH / boxW))
      : 1;
    const vCount = Math.max(10, Math.min(26, Math.round(hCount * ratio)));
    const dot = Math.max(2.8, width * 2.25);
    const points = [];

    for (let i = 0; i < hCount; i++) {
      const p = `${(i / hCount) * 100}%`;
      points.push({ top: -dot / 2, left: p, marginLeft: -dot / 2, width: dot, height: dot, borderRadius: dot });
    }
    for (let i = 0; i < vCount; i++) {
      const p = `${(i / vCount) * 100}%`;
      points.push({ right: -dot / 2, top: p, marginTop: -dot / 2, width: dot, height: dot, borderRadius: dot });
    }
    for (let i = 0; i < hCount; i++) {
      const p = `${(i / hCount) * 100}%`;
      points.push({ bottom: -dot / 2, right: p, marginRight: -dot / 2, width: dot, height: dot, borderRadius: dot });
    }
    for (let i = 0; i < vCount; i++) {
      const p = `${(i / vCount) * 100}%`;
      points.push({ left: -dot / 2, bottom: p, marginBottom: -dot / 2, width: dot, height: dot, borderRadius: dot });
    }
    return { points, dot };
  }

  function cometMotion(phase, index, count, trail) {
    if (!phase?.interpolate || count <= 1) return { opacity: 1, scale: 1 };
    const position = index / count;
    const length = trailFraction(trail);
    const samples = 72;
    const inputRange = Array.from({ length: samples + 1 }, (_, i) => i / samples);
    const opacityRange = inputRange.map(t => {
      const age = ((t - position) % 1 + 1) % 1;
      const headDistance = Math.min(age, 1 - age);
      const headWindow = Math.max(0.008, 0.8 / count);
      if (headDistance <= headWindow) return 1;
      if (age <= length) {
        const fade = 1 - age / length;
        return 0.05 + Math.pow(fade, 1.55) * 0.78;
      }
      return 0.025;
    });
    const scaleRange = inputRange.map(t => {
      const age = ((t - position) % 1 + 1) % 1;
      const headDistance = Math.min(age, 1 - age);
      const headWindow = Math.max(0.01, 1.1 / count);
      if (headDistance <= headWindow) return 1.85;
      if (age <= length * 0.28) return 1.2;
      return 0.82;
    });
    try {
      return {
        opacity: phase.interpolate({ inputRange, outputRange: opacityRange, extrapolate: "clamp" }),
        scale: phase.interpolate({ inputRange, outputRange: scaleRange, extrapolate: "clamp" }),
      };
    } catch {
  ...
  function brightnessHalo(outline, color, radius, phase) {
    ...
    if (outline.animation === "glow" && phase?.interpolate) {
      opacity = phase.interpolate({
        inputRange: [0, 0.5, 1],
        outputRange: maxMode ? [0.22, 0.72, 0.22] : [0.12, 0.52, 0.12],
      });
    }
    ...
    return React.createElement(Wrapper, {
      ...
      style: {
        ...
        borderWidth: outline.width + extra,
        borderColor: color,
        opacity,
      },
    });
  ...
  function OutlineVisual({ folder, state, baseStyle, fallbackRadius }) {
    ...
    }, [needsMotion, outline.speed, outline.animation, outline.colorMode, outline.trail]);
    ...
    const baseColor = outline.colorMode === "spectrum"
      ? animatedCycleColor(phase, FULL_SPECTRUM, 0)
      : outline.colorMode === "rgb"
        ? animatedCycleColor(phase, RGB_RING, 0)
        : outline.colorMode === "rainbow"
          ? animatedCycleColor(phase, RAINBOW_RING, 0)
          : outline.colorMode === "gradient"
            ? (outline.animation === "spin" ? animatedCycleColor(phase, outline.gradient, 0) : gradientAt(outline.gradient, 0))
            : outline.color;
    ...
    if (outline.animation === "chase") {
      const chase = chasePoints(outline.width, baseStyle);
      const baseOpacity = outline.brightness === "max" ? 0.3 : outline.brightness === "bright" ? 0.2 : 0.12;
      const haloExtra = outline.brightness === "max" ? 4.2 : outline.brightness === "bright" ? 3.2 : 2.3;
      ...
      return React.createElement(Wrapper, { pointerEvents: "none", style: containerStyle },
        ...
        chase.points.map((pointStyle, index) => {
          const offset = index / Math.max(1, chase.points.length);
          const motion = cometMotion(phase, index, chase.points.length, outline.trail);
          let pointColor = baseColor;
          if (outline.colorMode === "rgb") {
            pointColor = animatedCycleColor(phase, RGB_RING, offset * 0.18);
          } else if (outline.colorMode === "rainbow") {
            pointColor = animatedCycleColor(phase, RAINBOW_RING, offset * 0.35);
          } else if (outline.colorMode === "spectrum") {
            pointColor = animatedCycleColor(phase, FULL_SPECTRUM, 0);
          } else if (outline.colorMode === "gradient") {
            pointColor = animatedCycleColor(phase, modeColors, offset * 0.3);
          }
          ...
          return React.createElement(Wrapper, {
            ...
            style: {
              ...
              right: -haloExtra,
              bottom: -haloExtra,
              borderRadius: chase.dot + haloExtra * 2,
              backgroundColor: pointColor,
              opacity: outline.brightness === "max" ? 0.36 : outline.brightness === "bright" ? 0.25 : 0.16,
            },
          }));
        }),
      );
    ...
    const needsSegments = ["rgb", "rainbow", "gradient"].includes(outline.colorMode)
      || outline.pattern === "segmented"
      || outline.animation === "marquee"
    ...
    return React.createElement(Wrapper, { pointerEvents: "none", style: containerStyle },
      ...
      segments.map((style, index) => {
        ...
        if (outline.colorMode === "rgb") {
          segmentColor = animatedCycleColor(phase, RGB_RING, offset);
        } else if (outline.colorMode === "rainbow") {
          segmentColor = animatedCycleColor(phase, RAINBOW_RING, offset);
        } else if (outline.colorMode === "gradient") {
          segmentColor = outline.animation === "spin"
            ? animatedCycleColor(phase, modeColors, offset)
            : gradientAt(modeColors, offset);
        } else if (outline.colorMode === "spectrum") {
          segmentColor = animatedCycleColor(phase, FULL_SPECTRUM, 0);
        }

        const segmentOpacity = outline.animation === "marquee"
          ? marqueeOpacity(phase, index, segments.length)
          : 1;
      ...
      return after(method, target, (args, result) => {
        try {
          ...
          if (!expanded && cfg.cover === "folder") {
            transition.props.items = [{ type: "icon", tintStyle: { tintColor: cfg.coverAccent } }];
          }
          ...
          if (!expanded) {
            ...
            transition.props.wrapChildren = child => {
              ...
              try {
                ...
                if (tile?.props) {
                  const bg = React.createElement(BackgroundVisual, {
                    key: "tt-closed-bg",
                    folder,
                    baseStyle: tile.props.style,
                    fallbackRadius: 16,
                  });
                  const outline = React.createElement(OutlineVisual, {
                    key: "tt-closed-outline",
                    folder,
                    state: "closed",
                    baseStyle: tile.props.style,
                    fallbackRadius: 16,
                  ...
                  children[0] = React.cloneElement(
                    tile,
                    null,
                    appendVisuals(tile.props.children, bg, outline),
                  );
                }
              } catch (error) {
                try { console.error("[ThemeToolkit] collapsed visual failed", error); } catch {}
              }
              return wrapped;
            };
          }
        ...
        try {
          ...
          if (result?.props) {
            ...
            result = React.cloneElement(
              result,
              null,
              appendVisuals(result.props.children, bg, outline),
            );
          }
        } catch (error) {
          try { console.error("[ThemeToolkit] expanded folder visual failed", error); } catch {}
        }
        return result;
      });
    } catch (error) {
  ...
  function Settings() {
    ...
    function OutlineCard({ stateName, prefix }) {
      ...
      return React.createElement(RN.View, { style: card },
        ...
        React.createElement(RN.Text, { style: text },
          "RGB = rotating red/green/blue bands. Rainbow = many rainbow colors around the border. Full spectrum = one vivid color smoothly sweeps through the complete hue wheel."
        ),

        storage[modeKey] === "custom"
          ? React.createElement(ColorInput, { labelText: "Outline color", storageKey: `${prefix}OutlineColor` })
          : null,
        ...
        React.createElement(RN.Text, { style: text },
          "Chase = one bright comet/orb with a fading trail. Marquee = the older scanner-style moving bright sections."
        ),

        storage[animationKey] === "chase"
          ? React.createElement(RN.View, { style: { gap: 6 } },
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.3.2"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text },
          "Chase now uses a single comet-like bright point with a fading trail. The old multi-section chase is preserved as Marquee."
        ),
      ),
