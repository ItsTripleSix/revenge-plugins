(() => {
  ...
  function ChaseVisual({ outline, baseStyle, fallbackRadius }) {
    ...
    const solidLength = Math.max(dot * 0.95, pathSpacing * 1.24);
  ...
  function Settings() {
    ...
    return React.createElement(RN.ScrollView, { contentContainerStyle: page },
      React.createElement(RN.View, { style: card },
        React.createElement(RN.Text, { style: title }, "Theme Toolkit v0.3.9"),
        React.createElement(RN.Text, { style: text }, activeThemeText),
        React.createElement(RN.Text, { style: text }, "Solid Chase tails now use overlapping tangent-aligned capsules so long folders stay visually continuous without restoring the old heavy particle count. Solid Marquee and Color Spin now use the shared native perimeter clock instead of stepped JS color/segment transitions."),
      ),
      React.createElement(RN.View, { style: card },
